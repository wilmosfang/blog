...
前面一篇简单地对 ARM 裸机开发、平台环境与工具、创建项目、ADC(模数转换器)中断模式的控制与基础进行了演示
...
* 使用 ARM 板上的看门狗实现自动复位功能

#### 看门狗电路原理图

![wtd.png](/images/wtd/wtd.png)
...
运行效果是蜂鸣器会一直响,LED灯也会常亮

如果注释掉 `main.c` 中的 **`rWTCNT   = 50625;       //喂狗,如果不喂狗,超时就会复位`** 这一句,再次运行
...
![wtd_overview.png](/images/wtd/wtd_overview.png)
